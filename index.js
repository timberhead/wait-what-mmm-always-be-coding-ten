...
function options() {
  inquirer
    .prompt({
      ...
      type: 'list',
      message:
        'Welcome to your employee database. Choose one of the following options.',
      choices: [
...
function updateRole() {
  connection.query(
    'SELECT employees.id, employees.first_name, employees.last_name, role.title, role.id AS role_id FROM employees LEFT JOIN role ON role.id  = employees.role_id',
    function (err, response) {
      if (err) throw err
console.log(response);
      inquirer
        ...
        .then(function (answers) {
          ...
          connection.query(
            `UPDATE employees SET role_id = ${answers.roles}  WHERE id = ${answers.employees} `,
